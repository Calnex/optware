...
#	find $(ENDOR_IPK_DIR)/opt/ -type f -name "*.mdb" --delete
	export ENDOR_ALLOW_FROM=$(shell cat $(ENDOR_SOURCE_DIR)/allow-upgrade-from.regex 2>/dev/null)
	export ENDOR_RESTRICT_FROM=$(shell cat $(ENDOR_SOURCE_DIR)/restrict-upgrade-from.regex 2>/dev/null)
